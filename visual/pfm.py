...
def WriteMiddlebury2014CalibFile(path,
    ...
    with open(path, 'wb') as calib_file:
        calib_file.write(StrToBytes(
            'cam0=[' + str(left_fx) + ' 0 ' + str(left_cx) + '; 0 ' + str(left_fy) + ' ' + str(left_cy) + '; 0 0 1]\n'))
        calib_file.write(StrToBytes(
            'cam1=[' + str(right_fx) + ' 0 ' + str(right_cx) + '; 0 ' + str(right_fy) + ' ' + str(
                right_cy) + '; 0 0 1]\n'))
        ...
        for value in in_row:
            if math.isinf(value):
                ...
                if converted_value > 65535:
                    print(
                                'Warning: A disparity value of 256 or larger needed to be clamped in the conversion from PFM to Kitti PNG. File: ' + src_path)
...
def ConvertPfmNumpyToPng(pfm_numpy, dest_path):
    pfm_channels, pfm_height, pfm_width = pfm_numpy.shape
    data = pfm_numpy.reshape((pfm_height, pfm_width))
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            if math.isinf(data[i, j]):
                data[i] = 0
            else:
                data[i, j] = max(1, int(round(256.0 * data[i, j])))

    with open(dest_path, 'wb') as dest_png_file:
        png_writer = png.Writer(width=pfm_width, height=pfm_height, bitdepth=16, compression=9, greyscale=True)
        png_writer.write(dest_png_file, data)


def NumpyToPng(img_numpy, dest_path):
    r = Image.fromarray(img_numpy[0]).convert('L')
    g = Image.fromarray(img_numpy[1]).convert('L')
    b = Image.fromarray(img_numpy[2]).convert('L')
    wr = Image.merge("RGB", (r, g, b))
    wr.save(dest_path, 'png')
