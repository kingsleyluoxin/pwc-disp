...
caffe_bin = '/home/Working/caffe/build/tools/caffe.bin'
img_size_bin = '/home/Working/caffe/build/tools/get_image_size'

template = 'model/deploy.template.prototxt'
...
# Prepare prototxt
subprocess.call('mkdir -p tmp/tmp_input', shell=True)
subprocess.call('mkdir -p tmp/tmp_output', shell=True)
subprocess.call('mkdir -p tmp/tmp_model', shell=True)

if not using_lists:
    with open('tmp/tmp_input/img1.txt', "w") as tfile:
        tfile.write("%s\n" % img_files[0])

    with open('tmp/tmp_input/img2.txt', "w") as tfile:
        tfile.write("%s\n" % img_files[1])
else:
    subprocess.call(['cp', img_files[0], 'tmp/tmp_input/img1.txt'])
    subprocess.call(['cp', img_files[1], 'tmp/tmp_input/img2.txt'])
...
with open('tmp/tmp_model/deploy.prototxt', "w") as tfile:
    tfile.write(proto)

# Run caffe

trained_filenames = os.listdir('./training')
...
if len(trained_filenames)==0:
	...
	args = [caffe_bin, 
		'test', 
		'-model', 'tmp/tmp_model/deploy.prototxt',
		'-weights', './training/disp_iter_'+ str(latest_iter) + '.caffemodel',
